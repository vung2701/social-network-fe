...
describe('PostCard Component', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should display post stats correctly', () => {
      const store = createTestStore();
      const postWithStats = {
        ...mockPost,
        likes: 10,
        comments: [
          { id: '1', content: 'Comment 1', author: mockUser, createdAt: new Date().toISOString() },
          { id: '2', content: 'Comment 2', author: mockUser, createdAt: new Date().toISOString() },
        ],
      };

      render(
        <TestWrapper store={store}>
          <PostCard {...defaultProps} post={postWithStats} />
        </TestWrapper>
      );

      expect(screen.getByText('10 lượt thích • 2 bình luận')).toBeInTheDocument();
    });
  });
  ...
  describe('User Interactions', () => {
    ...
    it('should toggle comments visibility when toggle button is clicked', () => {
      const store = createTestStore();
      ...
      render(
        <TestWrapper store={store}>
          <PostCard {...defaultProps} post={postWithComments} />
        </TestWrapper>
      );
